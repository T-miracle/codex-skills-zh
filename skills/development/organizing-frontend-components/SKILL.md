...
目标是 Vue 且任务包含页面、组件或表单实现时，在完成上述判断后调用 `$writing-vue-components`；向它传递已确认的所有者、组件树和逻辑后缀，由其负责 Vue 表单、模板语法、接口证据与视觉实现门禁。
...
完成实现后，若本轮新增或修改了函数、class、实体字段或复杂流程，先使用 `$documenting-frontend-code` 生成或同步准确注释；随后运行项目原生的格式化、静态检查、测试和构建检查，并逐项确认：
